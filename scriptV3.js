 ...
 * Kolumny: Data | Nadawca | Temat | Treść
 */
function saveToHistory(senderEmail, subject, body) {
  try {
    ...
    sheet.appendRow([
      ...
      senderEmail,
      subject || "",
      (body || "").substring(0, 2000) // max 2000 znaków
    ]);
...
/**
 * Pobiera ostatnią wiadomość od danego nadawcy z arkusza historii.
 * Zwraca obiekt { subject, body } lub null jeśli brak historii.
 */
function findLastMessageBySender(senderEmail) {
  try {
    ...
    // Szukamy od końca — ostatni wpis od tego nadawcy
    for (var i = data.length - 1; i >= 1; i--) { // i>=1 pomija nagłówek
      var rowEmail = (data[i][1] || "").toString().toLowerCase().trim();
      if (rowEmail === senderEmail.toLowerCase().trim()) {
        result = {
          subject: (data[i][2] || "").toString(),
          body:    (data[i][3] || "").toString()
        };
        break;
      }
    }
  ...
  if (data.image && data.image.base64) {
    try {
      ...
      try {
        ...
        console.log("Wysłano obrazek 1 (czarno-biały) -> " + recipient);
      } catch (e) {
        MailApp.sendEmail({
          to:           recipient,
          subject:      "RE: " + subject + " [komiks]",
          htmlBody:     htmlBody1,
          inlineImages: inlineImages1,
          attachments:  attachments1,
          name:         "Komiks AI – Autoresponder"
      ...
      console.error("Błąd obrazka 1: " + e.message);
    }
  }

  // ── Obrazek 2: retro-pop lata 60. ────────────────────────────────────────
  if (data.image2 && data.image2.base64) {
    try {
      ...
      try {
        ...
        console.log("Wysłano obrazek 2 (retro-pop) -> " + recipient);
      } catch (e) {
        MailApp.sendEmail({
          to:           recipient,
          subject:      "RE: " + subject + " [retro]",
          htmlBody:     htmlBody2,
          inlineImages: inlineImages2,
          attachments:  attachments2,
          name:         "Komiks Retro – Autoresponder"
      ...
      console.error("Błąd obrazka 2: " + e.message);
    }
  }
}
...
function executeNawiazanieMailSend(data, recipient, subject, msg) {
  if (!data || !data.has_history || !data.reply_html) {
    return; // brak historii — cicho pomijamy
  }
...
function ___AAA_processEmails() {
  ...
  for (var i = 0; i < threads.length; i++) {
    ...
    // ── Pobierz historię z Google Sheets (dla nawiązania) ────────────────────
    ...
    var response = _callBackend(
      ...
      containsKeywordObrazek,  // wants_obrazek
      allAttachments,
      previousBody,            // previous_body (dla nawiązania)
      previousSubject          // previous_subject (dla nawiązania)
    );
    ...
    if (response && response.json) {
      ...
      if (!isBiz && !isAllowed && containsKeyword) {
        if (json.biznes) executeMailSend(json.biznes, fromEmail, subject, msg, "Notariusz – Informacja");
        if (json.zwykly) executeMailSend(json.zwykly, fromEmail, subject, msg, "Tyler Durden – Autoresponder");
      }
      // KEYWORDS2 → scrabble
