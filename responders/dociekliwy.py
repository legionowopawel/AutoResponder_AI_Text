...
def _buduj_html_email_pierwsza_gra(
    ...
    Poniżej: pełne drzewo decyzyjne — runda 1 + runda 2 ze wszystkimi opcjami i reakcjami Eryka.
    Nagłówek z pytaniem nadawcy: 2x mniejsza czcionka, zawijany tekst, mobile-friendly.
    """
    ...
    # ── Sekcja oryginalnej treści emaila — 2x mniejsza czcionka, zawijana ─────
    body_escaped = esc(body_original)
    body_html = (
        f"""<div style="background:#f5f0e8;border-left:3px solid #8b6914;padding:8px 12px;
  margin:0 0 14px;border-radius:0 4px 4px 0;font-family:Arial,sans-serif">
<div style="font-size:9px;color:#8b6914;font-weight:bold;letter-spacing:1px;margin-bottom:4px">
  &#9993; Twoja wiadomość:</div>
<div style="font-size:6px;color:#666;line-height:1.5;word-break:break-word;
  white-space:normal;overflow-wrap:break-word;max-width:100%">{body_escaped}</div>
</div>"""
        if body_escaped.strip()
        else ""
    )

    # ── Kolory dla opcji R1 i R2 ───────────────────────────────────────────────
    KOLORY_OPC = [
        ("#E6F1FB", "#185FA5", "#0C447C"),
        ("#E1F5EE", "#0F6E56", "#085041"),
        ("#FAEEDA", "#854F0B", "#633806"),
    ]
    # Przyciemnione warianty dla R2
    KOLORY_R2 = [
        ("#C8DCF5", "#0C447C", "#082E56"),
        ("#BDE8D8", "#085041", "#053628"),
        ("#F0D5B0", "#633806", "#3E2204"),
    ]
    LIT = ["A", "B", "C"]

    def _buduj_opcje_r2(r2: dict, indent_px: int = 0) -> str:
        """Buduje blok rundy 2 dla danej ścieżki wyboru R1."""
        if not r2:
            return ""
        pytanie_r2 = esc(r2.get("pytanie", ""))
        opcje_r2 = r2.get("opcje", {})
        if not pytanie_r2 and not opcje_r2:
            return ""

        opcje_r2_html = ""
        for li2, lit2 in enumerate(LIT):
            if lit2 not in opcje_r2:
                continue
            op2 = opcje_r2[lit2]
            tekst2 = esc(op2.get("tekst", lit2))
            reakcja2 = esc(op2.get("reakcja", ""))
            bg2, border2, col2 = KOLORY_R2[li2]
            reakcja2_html = (
                f"""<div style="font-size:8px;color:#7A6040;font-style:italic;
  border-top:1px dashed #C8A96A;margin-top:4px;padding-top:4px;line-height:1.4">{reakcja2}</div>"""
                if reakcja2
                else ""
            )
            opcje_r2_html += f"""<div style="flex:1;min-width:0;background:{bg2};border:1px solid {border2};
  border-radius:4px;padding:6px 7px;font-family:Arial,sans-serif">
  <div style="font-size:8px;font-weight:bold;letter-spacing:1px;color:{col2};margin-bottom:3px">{lit2})</div>
  <div style="font-size:9px;color:{col2};line-height:1.4">{tekst2}</div>
  {reakcja2_html}
</div>
"""

        return f"""<div style="margin-top:6px;margin-left:{indent_px}px;
  background:#F0E6FA;border:1px solid #7A5FB7;border-radius:5px;padding:7px 8px">
  <div style="font-size:8px;font-weight:bold;color:#534AB7;letter-spacing:1px;
    margin-bottom:5px;text-align:center">&#8627; RUNDA 2: {pytanie_r2}</div>
...
{opcje_r2_html}  </div>
    ...
    for pi, pytanie in enumerate(pytania, 1):
        ...
        for li, lit in enumerate(LIT):
            ...
            reakcja_op = esc(op.get("reakcja", ""))
            bg, border, col = KOLORY_OPC[li]

            reakcja_html = (
                f"""<div style="font-size:8px;color:#7A6040;font-style:italic;
  background:#FFF8F0;border:1px dashed #C8A96A;border-radius:3px;
  padding:5px 6px;margin-top:5px;line-height:1.4">{reakcja_op}</div>"""
                if reakcja_op
                else ""
            )

            r2_html = _buduj_opcje_r2(op.get("runda2", {}))

            opcje_html += f"""<div style="flex:1;min-width:120px;background:{bg};border:1.5px solid {border};
  border-radius:6px;padding:9px;font-family:Arial,sans-serif">
  <div style="font-size:9px;font-weight:bold;letter-spacing:1px;color:{col};margin-bottom:3px">{lit})</div>
  <div style="font-size:10px;color:{col};line-height:1.4">{tekst_op}</div>
  {reakcja_html}
  {r2_html}
</div>
"""

        pytania_html += f"""<div style="margin-bottom:20px">
<div style="background:#EEEDFE;border:2px solid #534AB7;border-radius:8px;
  padding:10px 16px;margin-bottom:8px;text-align:center;font-family:Arial,sans-serif">
  <div style="font-size:9px;color:#534AB7;font-weight:bold;letter-spacing:2px;margin-bottom:3px">PYTANIE {pi}</div>
  <div style="font-size:12px;color:#3C3489;font-weight:bold;line-height:1.4">{tresc}</div>
</div>
...
<div style="padding:12px;max-width:900px;margin:0 auto;box-sizing:border-box">

  <div style="background:#2C2C2A;color:#F1EFE8;text-align:center;padding:10px 16px;
    border-radius:8px;font-size:13px;font-weight:bold;margin-bottom:12px;letter-spacing:1px">
    ERYK RESPONDER&#8482; &middot; {pilne_badge}Drzewo Dociekliwo&#347;ci &middot; {esc(sn)}
  </div>
  ...
  <div style="background:#F0EEFE;border:1px solid #534AB7;border-radius:6px;
    padding:8px 12px;margin-bottom:14px;font-size:10px;color:#534AB7;text-align:center;
    font-style:italic">
    Pe&#322;ne drzewo decyzyjne Eryka &mdash; wszystkie &#347;cie&#380;ki i odpowiedzi:
  </div>
