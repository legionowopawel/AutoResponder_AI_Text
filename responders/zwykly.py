...
def _build_session_vars(
    ...
    vars_dict["USER_GENDER"] = {"M": "mezczyzna", "K": "kobieta"}.get(gender, "nieznana")
...
def _generate_cv_photo(body: str, cv_data: dict, test_mode: bool = False, gender: str = "N") -> str | None:
    """
    Generuje zdjęcie profilowe do CV przez FLUX.
    Prompt budowany lokalnie (bez AI) — oszczędność 1 calla.
    Zwraca base64 PNG lub None.

    gender: "M"/"K"/"N" z wykrywaczplci — steruje opisem postaci i strojem w prompcie FLUX.
    """
    try:
    ...
    # Pobierz szablony płci z JSON (z fallbackiem na N)
    gender_subjects = photo_cfg.get("gender_subject", {})
    gender_attires  = photo_cfg.get("gender_attire", {})
    gender_subject  = gender_subjects.get(gender, gender_subjects.get("N", "Portrait of a person,"))
    gender_attire   = gender_attires.get(gender,  gender_attires.get("N",  "stained formal business attire,"))

    tytul = cv_data.get("tytul_zawodowy", "") if cv_data else ""

    logger.info("[cv-photo] gender=%s → subject='%.60s'", gender, gender_subject)

    # Prompt budowany lokalnie — bez AI
    photo_prompt = (
        f"{gender_subject} {tytul or 'office worker'}, "
        f"{gender_attire} "
        f"facial bruises, black eye, split lip, messy hair, "
        f"Fight Club 1999 aesthetic, exhausted damaged look, "
...
def build_zwykly_section(
    ...
    if cv_data:
        cv_photo = _generate_cv_photo(body, cv_data, test_mode=test_mode, gender=gender)
