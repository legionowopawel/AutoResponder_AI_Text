...
# ─────────────────────────────────────────────────────────────────────────────
# POMOCNICZA: DANE NADAWCY — blok wstrzykiwany do każdego prompta
# ─────────────────────────────────────────────────────────────────────────────

def _build_nadawca_block(
    sender_name: str = "",
    sender_email: str = "",
    gender: str = "N",
    imie: str = "__BRAK__",
    ...
    """
    Buduje ujednolicony blok DANE_NADAWCY wstrzykiwany do każdego user-prompta.

    Zawiera gotowe zmienne do użycia przez AI:
      ZWROT_IMIENNY   — imię jeśli znane, inaczej nazwisko, inaczej "ten mężczyzna"/"ta kobieta"/"ta osoba"
      ZWROT_NAZWISKO  — samo nazwisko (do użycia przez wredny personel)
      ZWROT_PELNY     — imię + nazwisko razem (do użycia w nagłówkach formalnych)
      FORMA_PAN_PANI  — "Pan"/"Pani"/"Osoba"
      FORMA_TEN_TA    — "ten pacjent"/"ta pacjentka"/"ta osoba"
      GENDER_KOD      — "M"/"K"/"N" (surowy kod dla FLUX i innych modeli)
    """
    # ── Ustal dostępność imienia i nazwiska ──────────────────────────────────
    ma_imie     = imie and imie != "__BRAK__"
    ma_nazwisko = nazwisko and nazwisko != "__BRAK__"
    ma_fullname = sender_name and sender_name.strip()

    # ── Zwrot imienny (najczęstszy w treści raportu) ─────────────────────────
    if ma_imie:
        zwrot_imienny = imie
    elif ma_nazwisko:
        zwrot_imienny = nazwisko
    elif ma_fullname:
        zwrot_imienny = sender_name.split()[0]   # pierwsze słowo jako fallback
    else:
        # brak jakiegokolwiek imienia — zaimek rodzajowy
        if gender == "K":
            zwrot_imienny = "ta kobieta"
        elif gender == "M":
            zwrot_imienny = "ten mężczyzna"
        else:
            zwrot_imienny = "ta osoba"

    # ── Zwrot nazwiskowy (wredny personel: pielęgniarki, sprzątaczki) ────────
    if ma_nazwisko:
        zwrot_nazwisko = nazwisko
    elif ma_fullname and len(sender_name.split()) > 1:
        zwrot_nazwisko = sender_name.split()[-1]  # ostatnie słowo jako nazwisko
    elif ma_imie:
        zwrot_nazwisko = imie                      # imię zamiast nazwiska
    else:
        zwrot_nazwisko = zwrot_imienny

    # ── Pełna nazwa (nagłówki formalne, skierowania, karty) ──────────────────
    if ma_fullname:
        zwrot_pelny = sender_name
    elif ma_imie and ma_nazwisko:
        zwrot_pelny = f"{imie} {nazwisko}"
    else:
        zwrot_pelny = zwrot_imienny

    # ── Forma grzecznościowa ─────────────────────────────────────────────────
    if gender == "K":
        forma_pan_pani = "Pani"
        forma_ten_ta   = "ta pacjentka"
    elif gender == "M":
        forma_pan_pani = "Pan"
        forma_ten_ta   = "ten pacjent"
    else:
        forma_pan_pani = "Osoba"
        forma_ten_ta   = "ta osoba"

    return (
        f"═══ DANE NADAWCY (używaj tych zmiennych w całym raporcie) ═══\n"
        f"SENDER_NAME:      {sender_name or '__BRAK__'}\n"
        f"SENDER_EMAIL:     {sender_email or '__BRAK__'}\n"
        f"GENDER_KOD:       {gender}   (M=mężczyzna, K=kobieta, N=nieznana)\n"
        f"ZWROT_IMIENNY:    {zwrot_imienny}   ← używaj BARDZO CZĘSTO w treści raportu\n"
        f"ZWROT_NAZWISKO:   {zwrot_nazwisko}   ← pielęgniarki i sprzątaczki zwracają się PO NAZWISKU\n"
        f"ZWROT_PELNY:      {zwrot_pelny}   ← nagłówki formalne, skierowania, karty pacjenta\n"
        f"FORMA_PAN_PANI:   {forma_pan_pani}   ← lekarze i specjaliści mówią '{forma_pan_pani} {zwrot_imienny}'\n"
        f"FORMA_TEN_TA:     {forma_ten_ta}   ← gdy mówisz o pacjencie w 3. osobie\n"
        f"═══════════════════════════════════════════════════════════════\n"
        f"REGUŁY UŻYCIA:\n"
        f"• Imię pacjenta ({zwrot_imienny}) MUSI pojawić się w KAŻDEJ sekcji raportu min. 2 razy.\n"
        f"• Lekarze/specjaliści: '{forma_pan_pani} {zwrot_imienny}' lub '{zwrot_pelny}'.\n"
        f"• Pielęgniarki i sprzątaczki: WYŁĄCZNIE '{zwrot_nazwisko}' (wrednie, bez imienia).\n"
        f"• W nagłówkach formalnych (karta pacjenta, skierowanie): zawsze '{zwrot_pelny}'.\n"
        f"• Gdy mówisz o pacjencie w 3. osobie: '{forma_ten_ta}'.\n"
        f"══════════════════════════════════════════════════════════════\n"
    )
...
def _sekcja_pacjent(cfg: dict, body: str, sender_name: str, nadawca_block: str = "") -> dict:
...
def _sekcja_tydzien(
    cfg: dict, body: str, leki: list, tydzien: int, data_przyjecia: str, nadawca_block: str = ""
) -> list:
...
def _sekcja_wypis(cfg: dict, body: str, data_przyjecia: str, nadawca_block: str = "") -> dict:
    """Sekcja 5 — wypis."""
    try:
        ...
        user = (
            f"{nadawca_block}"
            f"EMAIL PACJENTA:\n{body[:MAX_DLUGOSC_EMAIL]}\n\n"
            f"DATA PRZYJECIA: {data_przyjecia}\n\n"
            f"INSTRUKCJE:\n{instrukcje}\n\n"
            f"SCHEMAT JSON:\n{json.dumps(schema, ensure_ascii=False, indent=2)}"
        )
...
def _sekcja_zalecenia(cfg: dict, body: str, dni_1_7: list, dni_8_14: list, nadawca_block: str = "") -> dict:
...
def _sekcja_leczenie_specjalne(
    cfg: dict, body: str, dni_1_7: list, dni_8_14: list, nadawca_block: str = ""
) -> dict:
...
def _sekcja_relacje_swiadkow(cfg: dict, body: str, raport: dict, nadawca_block: str = "") -> dict:
...
def build_raport(
    ...
    # ── Blok danych nadawcy — budowany raz, przekazywany do każdej sekcji ────
    # gender dla raport używa "M"/"K"/"N", a sygnatura przyjmuje "patient" jako fallback
    _gender_kod = gender if gender in ("M", "K") else "N"
    nadawca_block = _build_nadawca_block(
    ...
    current_app.logger.info(
        "[psych-raport] nadawca_block: gender=%s imie=%s nazwisko=%s sender=%s",
        _gender_kod, imie, nazwisko, sender_name,
    )
    ...
    try:
        sekcja_pacjent = _wrap_section(
            _sekcja_pacjent(cfg, body, sender_name, nadawca_block=nadawca_block),
            "pacjent",
        )
    except Exception as e:
    ...
    try:
        dni_1_7 = (
            _unwrap_section(
                _wrap_section(
                    _sekcja_tydzien(cfg, body, leki_lista, 1, data_przyjecia, nadawca_block=nadawca_block),
                    "tydzien1",
                )
            )
            or []
    ...
    try:
        dni_8_14 = (
            _unwrap_section(
                _wrap_section(
                    _sekcja_tydzien(cfg, body, leki_lista, 2, data_przyjecia, nadawca_block=nadawca_block),
                    "tydzien2",
                )
            )
            or []
    ...
    try:
        sekcja_wypis = _wrap_section(
            _sekcja_wypis(cfg, body, data_przyjecia, nadawca_block=nadawca_block),
            "wypis",
        )
    except Exception as e:
    ...
    try:
        sekcja_zalecenia = _wrap_section(
            _sekcja_zalecenia(cfg, body, dni_1_7, dni_8_14, nadawca_block=nadawca_block),
            "zalecenia",
        )
    except Exception as e:
    ...
    try:
        sekcja_leczenie_specjalne = _wrap_section(
            _sekcja_leczenie_specjalne(cfg, body, dni_1_7, dni_8_14, nadawca_block=nadawca_block),
            "leczenie_specjalne",
        )
    except Exception as e:
    ...
    relacje_result = {}
    try:
        relacje_result = _sekcja_relacje_swiadkow(cfg, body, raport, nadawca_block=nadawca_block) or {}
    except Exception as e:
