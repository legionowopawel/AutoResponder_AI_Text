...
def _get_prompt(text: str, n: int = 10, diff: str = "sredni") -> str:
    ...
    return PROMPT_TEMPLATE.replace("{text}", text).replace("{n}", str(n)).replace("{diff}", diff)
    ...
    for attempt in range(1, 4):
        ...
        try:
            r = requests.post(
                ...
                headers=hdrs, json=payload, timeout=90,
            )
            r.raise_for_status()
            return _parse_json(r.json()["choices"][0]["message"]["content"])
        except json.JSONDecodeError as e:
...
class _PDF:
    ...
    def sf(self, b=False): return FB if b else FN
    def ff(self):          return "Helvetica"
    ...
    def textfield(self, name, x, y, w, h, val="", bs="solid",
                  fc=None, bc=None, fs=10, ro=False):
        kw = dict(name=name, x=x, y=y, width=w, height=h,
                  value=val, borderStyle=bs, forceBorder=True,
                  fontName=self.ff(), fontSize=fs)
        if fc: kw["fillColor"]   = fc
        if bc: kw["borderColor"] = bc
        if ro: kw["fieldFlags"]  = "readOnly"
        try:
            self.f.textfield(**kw)
        except Exception:
            try:
                self.f.textfield(name=name, x=x, y=y, width=w, height=h,
    ...
    def wrap(self, x, y, text, bold=False, sz=9,
             maxw=None, lh=5.2, col=None) -> float:
        fn = self.sf(bold)
        mw = maxw or (PW - x - ML)
        lines = simpleSplit(text, fn, sz, mw)
        self.c.saveState()
        self.c.setFont(fn, sz)
        if col: self.c.setFillColor(col)
        for ln in lines:
    ...
    def hint_annotation(self, q, qnum):
        ...
        c.saveState()
        c.setFillColor(CY)
        c.circle(icon_x + icon_size/2, icon_y - icon_size/2, icon_size/2, fill=1, stroke=0)
        c.setStrokeColor(CYB)
        c.circle(icon_x + icon_size/2, icon_y - icon_size/2, icon_size/2, fill=0, stroke=1)
        c.setFont(self.sf(True), 8)
        c.setFillColor(HexColor("#7d4e00"))
        c.drawCentredString(icon_x + icon_size/2, icon_y - icon_size/2 - 2.5*mm,
                            "?")
        c.restoreState()
    ...
    def cover(self):
        ...
        self.textfield("imie", m+50*mm, dy-3*mm, w-2*m-50*mm, 8*mm,
        ...
        self.textfield("wynik", m+22*mm, dy-4*mm, 50*mm, 8*mm,
                       fc=HexColor("#eafaf1"), bc=CG)
        c.drawString(m+75*mm, dy, "pkt")

        c.drawString(m+90*mm, dy, "Ocena:")
        self.textfield("ocena", m+108*mm, dy-4*mm, 30*mm, 8*mm,
                       fc=HexColor("#eafaf1"), bc=CG)

        c.setFont(self.sf(), 8)
        c.setFillColor(CM)
        c.drawString(m+5*mm, dy-13*mm,
    ...
    def qhead(self, q, qnum):
        ...
        c.setFillColor(white)
        c.drawString(m+3*mm, self.y-5*mm,
                     f"Pytanie {qnum}  \u2022  {ql}  \u2022  {pts} pkt")
        self.y -= BH + 3*mm

        # Treść pytania (z marginesem na ikonę ?)
        qtxt = f"{qnum}. {q.get('question', '')}"
        self.y = self.wrap(m+2*mm, self.y, qtxt,
                           sz=10, col=CT, maxw=w-2*m-12*mm, lh=5.5)
        self.y -= 2*mm
    ...
    def key(self):
        ...
        c.setFillColor(white)
        c.drawCentredString(w/2, self.y-8.5*mm,
                            "KLUCZ ODPOWIEDZI  \u2013  tylko dla prowadzacego")
        self.y -= 20*mm
        ...
        for i, q in enumerate(self.exam.get("questions", []), 1):
            ...
            pts = q.get("points", 1)
            qt  = {"multiple_choice": "MC", "true_false": "TF"}.get(q.get("type", ""), "?")
    ...
    def build(self):
        ...
        for i, q in enumerate(qs, 1):
            t = q.get("type", "multiple_choice")
            self.ensure(30)
            if   t == "multiple_choice": self.mc(q, i)
            elif t == "true_false":      self.tf(q, i)
        self.key()
        self.c.save()
